...
void game_sv_Single::Perform_game_export	()
{
	struct NetExportToClientFunctor
	{
		game_sv_Single* server_ptr;
		NetExportToClientFunctor(game_sv_Single* server) :
			server_ptr(server)
		{
		}
...
void game_sv_Single::Export_game_type(IClient* CL)
{
	NET_Packet			P;
	u32					mode = net_flags(TRUE,TRUE);
